 ...
 *
 * <p>
 * {@link InMemorySessionPerformanceTests} runs the same harness against the bundled
 * in-memory backend, and the two together are what separates the adapter's own cost from
 * etcd's.
 */
@Tag("performance")
@TestMethodOrder(MethodOrderer.OrderAnnotation.class)
@SpringBootTest(classes = EtcdSessionPerformanceTests.SessionApplication.class,
		properties = { "redis-adapter.backend=etcd" })
class EtcdSessionPerformanceTests {
	...
	private SessionPerformanceHarness harness() {
		return new SessionPerformanceHarness(this.sessions, report, "etcd", ATTRIBUTE_BYTES, endpoint);
	}
