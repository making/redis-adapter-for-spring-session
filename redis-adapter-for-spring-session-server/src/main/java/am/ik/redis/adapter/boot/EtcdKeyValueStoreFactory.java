 ...
 *
 * <p>
 * Nothing is connected to until {@link #create(int)} is called, which is what lets this
 * be registered whether or not it is the backend an operator asked for. The TLS bundle is
 * resolved there for the same reason: a server that keeps its sessions in memory must not
 * fail to start because a bundle it never uses is misspelt.
 *
...
public record EtcdKeyValueStoreFactory(EtcdBackendProperties properties,
	...
	@Override
	public String name() {
		return RedisAdapterProperties.ETCD_BACKEND;
	}
