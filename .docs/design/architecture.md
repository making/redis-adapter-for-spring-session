...
- Beyond Spring Session: `SET` (with `EX`/`PX`/`EXAT`/`PXAT`, whose deadline the backend writes
  with the value in one operation) and `GET`, so that a backend can be tried out with a
