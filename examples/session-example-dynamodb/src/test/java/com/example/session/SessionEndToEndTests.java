 ...
 * so the active profile decides that: an adapter each over one shared emulated DynamoDB
 * by default, and one real Redis behind both under
 * {@code -Dspring.profiles.active=redis}. Both are expected to pass identically — Redis
 * is what Spring Session was written against, so a test that passes there and fails
 * against the adapter is the adapter's fault.
 *
 * <p>
...
class SessionEndToEndTests {
	...
	private static ConfigurableApplicationContext startApplication(String instanceName) {
		return new SpringApplicationBuilder(SessionExampleDynamoDbApplication.class)
